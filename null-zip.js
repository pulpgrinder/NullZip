...
// Generate an entry for the zip central directory
  generateZipCentralRecord(zipentry){
  ...
  finalizeZip(zipbuffer){
    ...
    for(var i = 0; i < nfiles; i++){
      ...
      centralRecord = this.generateZipCentralRecord(zipbuffer[i])
...
// the default (0644).
  addTextFileToZip(zipbuffer, filename,textdata, timestamp, permissions){
    timestamp = timestamp || (new Date()).getTime() / 1000;
    permissions = permissions || 0o644;
    // Encode the text so this (hopefully) handles Unicode.
    return this.addFileToZip(zipbuffer,filename,new TextEncoder().encode(textdata), timestamp, permissions)
  }
...
// Permissions should be the Unix file permission mask or null for
// the default (0644).
  addFileToZip(zipbuffer,filename,dataArray,timestamp,permissions){
    timestamp = timestamp || (new Date()).getTime() / 1000;
    permissions = permissions || 0o644; // The default, corresponds to -rw-r--r
    let zipEntry = {};
